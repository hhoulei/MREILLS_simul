...
MRMETA <- function(fdata_all){
  ...
  for(ok in 1:ncol(mr_beta1)){
    ...
    x.inv <- try(metagen(mr_beta5[,ok],mr_se5[,ok]),
                 silent = T)
    if('try-error' %in% class(x.inv)){
  ...
  colnames(mrmeta1) <- colnames(mrmeta2) <- colnames(mrmeta3) <- 
    colnames(mrmeta4) <- colnames(mrmeta5) <- c('TE.common','pval.common',
  ...
  mrmeta <- list(mrmeta1=mrmeta1,
                 ...
                 mrmeta4=mrmeta4,
                 mrmeta5=mrmeta5)
  names(mrmeta) <- c('MVIVW','MVEgger','MVLasso','MVMedian','MVcML')
  return(mrmeta)
}
...
METAMR <- function(fdata_all){
  ...
  #res5 <- data.frame(res15@Estimate,res15@Pvalue,res25@Estimate,res25@Pvalue) 
  res6 <- data.frame(res16$est,res16$pval,res26$est,res26$pval)
  
  colnames(res1) <- colnames(res2) <- colnames(res3) <- 
    colnames(res4) <- colnames(res5) <- colnames(res6) <- 
  ...
  metamr <- list(res1=res1,
                 ...
                 res5=res5,
                 res6=res6)
  names(metamr) <- c('MVIVW','MVEgger','MVLasso','MVMedian','MVcML','MVBMA')
  return(metamr)
}
